...
### ShopeeFood adapter — implementation approach (validated by spike)

ShopeeFood's shop page is client-rendered (a plain HTTP fetch returns no
usable content), so this was investigated directly with a real browser
before writing the plan:

- The page's own frontend calls two JSON endpoints while loading:
  `GET https://gappapi.deliverynow.vn/api/delivery/get_detail?id_type=1&request_id={shopId}`
  (shop info) and
  `GET https://gappapi.deliverynow.vn/api/dish/get_delivery_dishes?id_type=2&request_id={internalDeliveryId}`
  (menu, grouped by category) — both confirmed returning real, complete
  data for this shop (`"result":"success"`) when observed through a real
  browser session.
- **Calling these endpoints directly (plain `fetch`, no browser) was
  tried and returns `403` with an anti-bot error body.** The real
  browser's requests carry several obfuscated, per-session fingerprint
  headers (e.g. randomly-named hashed header keys, an `x-sap-ri` risk
  token) generated by the site's own client-side JS. Reverse-engineering
  and replicating those headers to make the API accept unauthenticated
  server-side requests would mean deliberately defeating ShopeeFood's
  anti-bot protection — **out of scope, not something this feature
  builds**, regardless of it being the shop owner's own listing.
- The legitimate path, and the one this feature uses: drive a **real
  Playwright browser** to `sourceUrl`, and **listen for the two responses
  above as the page naturally loads** (`page.waitForResponse`), then
  parse their JSON bodies directly — no DOM scraping/selectors needed at
  all, since the page's own network traffic already carries fully
  structured data. This is standard browser automation (a real browser,
  executing the site's real JS, as any visitor's browser would) rather
  than spoofing a fingerprint to talk to the API standalone.

`playwright` is already a devDependency (used for e2e tests) — for
production use it must move to `dependencies`, and the Dockerfile needs
the Chromium browser binary installed
(`npx playwright install --with-deps chromium`), which increases image
size — called out explicitly since this is a self-hosted build.

Confirmed response shapes to parse (field names used verbatim in the
plan's adapter code):

- Shop detail → `reply.delivery_detail`: `.name`, `.address`,
  `.delivery_id` (numeric id used as `request_id` for the dishes call),
  `.logo_mms_img_id` (image id — full URL is
  `https://mms.img.susercontent.com/{id}@resize_ss{w}x{h}!@crop_w{w}_h{h}_cT`),
  `.photos[]` (array of `{width, height, value}`, `value` already a full
  banner image URL — pick the largest `width`), `.time.week_days[]`
  (array of `{week_day, start_time, end_time}`).
- Dishes → `reply.menu_infos[]`, each `{dish_type_name, dishes[]}`; each
  dish: `.id`, `.name`, `.description`, `.price.value` (VND, number),
  `.is_available` (boolean), `.photos[]` (same `{width, value}` shape as
  above).

Robustness requirements for the adapter:
- Normal browser User-Agent, reasonable request pacing (single run/day,
  no burst requests).
- `page.waitForResponse(...)` on a bounded timeout for both endpoints; a
  timeout, a non-200 status, or `result !== "success"` in either body all
  throw a typed error that `sync-service.ts` catches and records as a
  failed run — the browser is always closed (`try`/`finally`) and a
  scraping failure must never crash the server process.
